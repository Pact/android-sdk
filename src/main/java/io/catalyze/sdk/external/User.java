 ...
 * <ol>
 *     <li>Verify the user's username and password using {@link #checkUsernameFormat(String)} and
 *     {@link #checkPasswordFormat(String)} respectively.</li>
 *     <li>Create a new {@code User} instance.</li>
 *     <li> If the user is a returning user, login the user by calling
 *     {@link #login(com.android.volley.RequestQueue, com.android.volley.Response.Listener,
 ...
 * To cancel a request, call {@code Request.cancel()}
 */
public class User extends CatalyzeObject {
     ...
     * @throws IllegalUsernameFormatException if the username provided does not meet the catalyze.io
     *                                        API format requirements. Consider using {@link
     *                                        #checkUsernameFormat(String)} before constructing.
     * @throws IllegalPasswordFormatException if the password provided does not meet the catalyze.io
     *                                        API format requirements. Consider using {@link
     *                                        #checkPasswordFormat(String)} before constructing.
     * @see #checkPasswordFormat(String)
     * @see #checkUsernameFormat(String)
     */
    public User(String username, String password) throws IllegalUsernameFormatException, IllegalPasswordFormatException {
        super();
    ...
    /**
     * Creates this {@link User} on the catalyze.io API backend. This {@link User}
     * is not modified as a result of this method. If an error occurs performing the HTTP
     * interaction, the interaction is not retried. This {@link User} is not cached for
     * future upload.
     *
    ...
    public Request create(RequestQueue queue, Response.ErrorListener errorListener, final Person person) {
    ...
    /**
     * Updates this {@link User} on the catalyze.io API backend. This overwrites any
     * previous data stored for this user. If an error occurs performing the HTTP interaction, the
    ...
    /**
     * Deletes this {@link User} from the catalyze.io API backend. Also deletes the
     * associated {@link Person} on the backend (but leaves the local copy, if one exists, intact.)
     * If an error occurs performing the HTTP interaction, the interaction is not retried.
     *
     * @param queue         a {@link RequestQueue} from the Volley android library. This queue will
     *                      handle the HTTP interaction with the catalyze.io API servers.
     * @param errorListener to handle errors that may occur during the server interaction. A {@code
    ...
    /**
     * Logs this {@link User} into the catalyze.io API backend. If an error occurs
     * performing the HTTP interaction, the interaction is not retried.
     *
     ...
     * @return the {@link Request} representing this server interaction
     */
    public Request login(RequestQueue queue, Response.Listener<Person> listener, Response.ErrorListener errorListener) {
        return authenticate(queue, listener, errorListener, sBasePath + sSessionPath, mContent);
    }

    /**
     * Revokes this {@link User}'s  session key from the catalyze.io API backend. This
     * {@link User} is not modified as a result of this method, however future interactions
     * will require a call to {@link #login(com.android.volley.RequestQueue,
     * com.android.volley.Response.Listener, com.android.volley.Response.ErrorListener)}. If an
     * error occurs performing the HTTP interaction, the interaction is not retried.
     *
    ...
    public Request logout(RequestQueue queue, Response.ErrorListener errorListener) {
        ...
        CatalyzeJsonObjectRequest request = new CatalyzeJsonObjectRequest(Request.Method.DELETE, sBasePath + sSessionPath, null, listener, errorListener);
        queue.add(request);
        return request;
    }

    private CatalyzeJsonObjectRequest authenticate(RequestQueue requestQueue, final Response.Listener<Person> listener, Response.ErrorListener errorListener, String path, Map<String, Object> content) {
    ...
    /**
     * Do not use this method. Calls to this method will simply be dropped as {@link User}s
     * do not have optional fields.
     *
     * @param k do not use
     * @param v do not use
     * @return null
