...
/**
 * A {@link OldCatalyzeObject} used to represent Users on the catalyze.io API.
 *
 ...
 */
public class OldUser extends OldCatalyzeObject {
    ...
    /**
     * Creates this {@link OldUser} on the catalyze.io API backend. This {@link OldUser} is not modified
     * as a result of this method. If an error occurs performing the HTTP interaction, the
     * interaction is not retried. This {@link OldUser} is not cached for future upload.
     *
    ...
    public Request create(
        ...
        OldCatalyzeJsonObjectRequest request =
                new OldCatalyzeJsonObjectRequest(Request.Method.POST, sBasePath + sPath,
                        new JSONObject(content), new Response.Listener<JSONObject>() {

                    @Override
                    public void onResponse(JSONObject object) {

                        OldCatalyzeObject.sSessionToken =
                                (String) object.remove(OldCatalyzeObject.sSessionId);
    ...
    /**
     * Updates this {@link OldUser} on the catalyze.io API backend. This overwrites any previous data
    ...
    public Request update(RequestQueue queue, Response.ErrorListener errorListener) {
        ...
        OldCatalyzeJsonObjectRequest request = new OldCatalyzeJsonObjectRequest(Request.Method.PUT,
                sBasePath + sPath, new JSONObject(mContent), new Response.Listener<JSONObject>() {

            @Override
            public void onResponse(JSONObject object) {

                OldCatalyzeObject.sSessionToken = (String) object.remove(OldCatalyzeObject.sSessionId);

                // TODO: do we really need to inflate? what is returned? if not, use parent method.
                OldUser.this.inflateFromJson(object);
            }
        }, errorListener);
    ...
    /**
     * Deletes this {@link OldUser} from the catalyze.io API backend. Also deletes the associated
    ...
    /**
     * Logs this {@link OldUser} into the catalyze.io API backend. If an error occurs performing the
    ...
    /**
     * Revokes this {@link OldUser}'sTransactionPath  session key from the catalyze.io API backend. This {@link OldUser}
    ...
    public Request logout(RequestQueue queue, Response.ErrorListener errorListener) {
        ...
        OldCatalyzeJsonObjectRequest request = new OldCatalyzeJsonObjectRequest(Request.Method.DELETE,
                sBasePath + sSessionPath, null, listener, errorListener);
        queue.add(request);
        return request;
    }

    private OldCatalyzeJsonObjectRequest authenticate(RequestQueue requestQueue,
            final Response.Listener<Person> listener, Response.ErrorListener errorListener,
            String path, Map<String, Object> content) {

        OldCatalyzeJsonObjectRequest request = new OldCatalyzeJsonObjectRequest(Request.Method.POST, path,
                new JSONObject(content), new Response.Listener<JSONObject>() {
     ...
     * @return this {@link OldUser} instance
     * @throws IllegalPasswordFormatException if the catalyze.io API format requirements for
     *                                        passwords are not met
     * @see OldUser#checkPasswordFormat(String) for format requirements
     */
    public OldUser setPassword(String password) throws IllegalPasswordFormatException {
     ...
     * @see OldUser#checkUsernameFormat(String) for format requirements
     */
    public OldUser setUsername(String username) throws IllegalUsernameFormatException {
    ...
    /**
     * Do not use this method. Calls to this method will simply be dropped as {@link OldUser}sTransactionPath do not
     ...
     */
    @Override
    public OldCatalyzeObject put(String k, Object v) {
        return null;
    }
