...
public class CatalyzeUser extends CatalyzeObject implements Comparable<CatalyzeUser> {
     ...
     * @param context
     */
    protected void getAuthenticatedUser(String userName, String password, CatalyzeListener<CatalyzeUser> callbackHandler, Context context){
    ...
    /***
     * TODO
     * Pushes any changes made to this instance of user to server
     * @param context
     */
    public void update(CatalyzeListener<CatalyzeUser> callbackHandler, Context context){
		Map<String, String> headers = getAuthorizedHeaders();
		responseListener = createListener(callbackHandler);
		errorListener = createErrorListener(callbackHandler);
		...
		do {
			String k = keys.next();
			if(!jsonBody.isNull(k)){
				updates.put(k, jsonBody.get(k));
			}
		} while (keys.hasNext());
		} catch (JSONException e) {
    ...
    protected void signUp(String userName, String password, String firstName, String lastName, CatalyzeListener<CatalyzeUser> callbackHandler,  Context context) {
    ...
    /**
     * Signup new user with the information in this user and the provided password
     * @param password 
     * @param callbackHandler 
     * @param context 
     */
	...
	/**
	 * Sign out this user
	 * @param callbackHandler Funtion to call after HTTP response handled
	 * @param context
	 */
	protected void signOut(CatalyzeListener<CatalyzeUser> callbackHandler,  Context context){
		Map<String, String> headers = getAuthorizedHeaders();
		responseListener = createSignoutListener(callbackHandler, this);
		errorListener = blankResponseErrorHandler(callbackHandler);
		JSONObject json = new JSONObject();
		CatalyzeRequest request = new CatalyzeRequest(SIGOUT_URL, json, responseListener, errorListener);
		request.setHeaders(headers);
		request.get(context);
	}
	...
	protected void getUserInfo(String userName, CatalyzeListener<CatalyzeUser> callbackHandler,  Context context){
		...
		errorListener = createErrorListener(callbackHandler);
		CatalyzeRequest request = new CatalyzeRequest(USER_ROUTE + "/" + userName, null, responseListener, errorListener);
		request.setHeaders(headers);
		request.get(context);
	}
	...
	private Response.Listener<JSONObject> createSignoutListener(
			final CatalyzeListener<CatalyzeUser> userCallback, final CatalyzeUser user) {
		return new Response.Listener<JSONObject>() {
			@Override
			public void onResponse(JSONObject response) {
				user.setJson(null);
				user.setUserSessionToken(null);
    ...
    /***
	 * FIXME Maybe figure out a better way around this bug?
	 * Specialized error listener for use in delete and signout path to deal with "End of input" json error
	 * @return
	 */	
	private Response.ErrorListener blankResponseErrorHandler(final CatalyzeListener<CatalyzeUser> userCallback) {
		return new Response.ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
				...
				if(error.getMessage() != null && error.getMessage().equals("org.json.JSONException: End of input at character 0 of ")){
					setJson(null);
					setUserSessionToken(null);
					userCallback.onSuccess(CatalyzeUser.this);
				}
				else userCallback.onError(ce);
			}
		};
	}
	 ...
	 * @return
	 */	
	private Response.ErrorListener createErrorListener(final CatalyzeListener<CatalyzeUser> userCallback) {
		return new Response.ErrorListener() {
